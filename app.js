...
function generateAllQRCodes() {
  ...
  if (typeof QRCode === 'undefined') {
    ...
    ['admin', 'guru', 'siswa'].forEach(role => {
      ...
      if (container) {
        container.innerHTML = '<div style="text-align: center; padding: 40px; color: #EF4444; font-size: 14px;">❌ QR Code library not loaded. Please refresh the page.</div>';
      }
    });
    return;
  }
  ...
  roles.forEach(role => {
    ...
    setTimeout(() => {
      try {
        ...
        if (qrCodeInstances[role]) {
          try {
            qrCodeInstances[role].clear();
          } catch (e) {
            console.log('Previous QR instance cleanup skipped');
          }
        }
        ...
        setTimeout(() => {
          ...
          if (!canvas && !img) {
            console.error(`❌ QR Code canvas/img not found for ${role}`);
            container.innerHTML = '<div style="text-align: center; padding: 40px; color: #F59E0B; font-size: 14px;">⚠️ QR Code generated but not visible. Try refreshing.</div>';
          } else {
            console.log(`✓ QR Code visible for ${role}:`, canvas ? 'canvas' : 'img');
          }
        }, 200);
      } catch (error) {
        ...
        container.innerHTML = `<div style="text-align: center; padding: 40px; color: #EF4444; font-size: 14px;">
          ...
          <button class="btn btn-sm btn-secondary" onclick="generateAllQRCodes()" style="margin-top: 12px;">🔄 Retry</button>
        </div>`;
      }
    }, 150 * (roleIndex + 1)); // Stagger generation
  });
